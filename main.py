...
def part1(file_name: str, is_test: bool = False):
    try:
        data = utils.load_data(filename=file_name)

        if is_test:
            data_after = utils.preprocess(data)
        else:
        ...
        X = data_after[relevant_features]
...
if __name__ == '__main__':
    ...
    num_of_lines = np.count_nonzero((y_pred >= 1).any(axis=1))
    num_overall = np.count_nonzero(y_pred >= 1)
